...
class SiidUriHandler {
    ...
    async getOrgInfoFromAuthenticated() {
        try {
    ...
    async getExistingProject(orgName) {
        const projectMappings = this.context.globalState.get('projectMappings', {});
        return projectMappings[orgName];
    }
    async storeProjectMapping(orgName, projectPath) {
    ...
    async createNewProjectFolder(orgName) {
    ...
    async authenticateOrg(sessionId, instanceUrl) {
        this.log('Authenticating Salesforce org...');
