...
A Chrome extension that extracts Salesforce session IDs and opens them in the SIID protocol handler with a single click.
...
3. **One-Click Integration**: Click the extension icon to immediately open the `siid://` protocol handler with the session ID
4. **Automatic Icon State**: Extension icon is enabled only on Salesforce domains, disabled elsewhere
